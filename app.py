...
def fetch_tmdb_data(api_key, query, language):
    """
    Calls the TMDB API using the selected language (en-US or ar-SA).
    """
    if not api_key:
        return pd.DataFrame()
    ...
    # Set the API language parameter based on user selection
    ...
    # English and Arabic dictionaries for movie genres
    genre_map_en = {
...
def get_ai_recommendations(user_query, df):
    ...
    vectorizer = TfidfVectorizer() # Removed 'english' stop_words so it works with Arabic text too
